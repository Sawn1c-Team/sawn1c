...
def run_sandboxed_build(name: str, author: str, stage_dir: str) -> bool:
    ...
    env = {"PATH": "/usr/bin:/bin:/usr/local/bin", "DESTDIR": stage_dir, "HOME": "/tmp"}
    ...
    for cmd in cmds:
        if not cmd:
            continue
        try:
            subprocess.run(["unshare", "-n", "sh", "-c", cmd], env=env, check=True)
...
if __name__ == "__main__":
    print(
        "This module is a component of the package manager and should not be run directly."
    )
